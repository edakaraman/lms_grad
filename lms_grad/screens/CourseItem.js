...
export default function CourseItem({ item,completedChapter,progress,isPopular }) {
  ...
  return (
    <View className="p-2 bg-white mr-3 rounded-2xl">
      ...
      />
      <View className="p-2 pb-0 justify-between flex flex-row">
        <Text className="text-lg"> {item.name} {""} {isPopular && <Text className="text-yellow-500 text-2xl font-bold">★</Text>}  </Text>
      </View>
